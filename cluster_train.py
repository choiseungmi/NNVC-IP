...
def main(args):
    ...
    cluster_log = Logger(os.path.join('log_csv\\clusters', str(args.height)+"x"+str(args.width)+"_"+str(args.quality)+'.csv'))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_dataset = clustering.cluster_assign(deepcluster.images_lists,
                                                   os.path.join(args.data,  str(args.quality), str(args.height)+"x"+str(args.width)))
        ...
        torch.save({'epoch': epoch + 1,
                   ...
                   os.path.join(config.cluster_checkpoint, str(args.height)+"x"+str(args.width)+"_" + str(args.quality) + '.pth.tar'))
    ...
    for i, (input_tensor, target) in enumerate(loader):
        ...
        if n % args.checkpoints == 0:
            path = os.path.join(
                args.exp,
                'checkpoint',
                str(args.height)+"x"+str(args.width)+"_" + str(args.quality) + str(n / args.checkpoints) + '.pth.tar',
            )
            if args.verbose:
