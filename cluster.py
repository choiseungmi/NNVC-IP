...
def main(args):
    ...
    images = prepare_image(path_to_files, model, args.height, args.height)
    km = km.fit(images)
    save_cluster(path_to_save, km)

    images = prepare_image(os.path.join(config.valid_numpy_path,  str(args.quality), str(args.height)+"x"+str(args.width)), model, args.height, args.height)
...
def prepare_image(path_to_files, model, h, w):
    # size = (224, 224)
    tra = [transforms.ToTensor()]
    #imgs_dict = ClusterDataset(path_to_files, h, w, transform=transforms.Compose(tra))
