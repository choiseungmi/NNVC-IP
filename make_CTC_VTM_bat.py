...
def main(file_path, cfg, path_input, path_bitstream, frame_rate, frame):
    ...
    for qp in qp_list:
        ...
        enc_line = "EncoderApp.exe -c encoder_intra_vtm.cfg -c "+cfg+" -i input\\"+path_input+" -b "+path_bitstream+" -q "+qp+" -fr "+frame_rate+" -f "+frame+" --ReconFile="+output_path+"\\recon\\"+path_recon_file+" -fs 0 > "+output_path+"\\encoder\\"+path_log+"\n"
