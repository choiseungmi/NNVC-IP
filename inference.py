...
from make_dataset.load_dataset import AIHUB_INFERENCE
    ...
    with torch.no_grad():
        for data, _, name in tqdm(data_load):
            ...
            pred = torch.IntTensor(
                [pred2[i] if pred[i] == 0 and value < math.log(0.7) else pred[i] for i, value in enumerate(_)]).cuda()
...
def inference_none(model, path, target):
    ...
    test_loader = DataLoader(hmdb51_test, batch_size=opts.BATCH_SIZE, shuffle=False)
